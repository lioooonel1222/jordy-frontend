...
const JordyChatbot: React.FC = () => {
  ...
  // ✅ Ref explizit typisieren
  const messagesEndRef = useRef<HTMLDivElement | null>(null);

  const scrollToBottom = () => {
    const el: HTMLDivElement | null = messagesEndRef.current;
  ...
  const handleSendMessage = async () => {
    ...
    // Demo-Antwort (später Backend-API-Call)
    setTimeout(() => {
      const botMessage: ChatMessage = {
        id: Date.now() + 1,
        role: 'assistant',
        content: `Hi! Ich bin Jordy und habe verstanden: "${userMessage.content}". In Zukunft route ich zwischen GPT-4o-mini und Claude.`,
        timestamp: new Date(),
      };
                  ...
                  <div className="flex items-end space-x-2">
                    ...
                    <div className="flex space-x-1">
                      <button
                        ...
                        aria-label="Spracheingabe (später)"
                      >
                        <Mic size={16} />
                      </button>
                      <button
                        className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                        aria-label="Audio stumm (später)"
                      >
                        <VolumeX size={16} />
                      </button>
                    </div>
